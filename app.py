...
# --------------------------
# GOOGLE_API_KEY = "AIzaSyD85wx91fLGMHwxFyvSQzoPzjW2HFSxeZY" # This should ideally be fetched from secrets
GOOGLE_API_KEY = "AIzaSyAcfALCP0WR8qA0sCZBKPVjni-3GdaZQP4"

if not GOOGLE_API_KEY:
    st.warning("⚠️ Google API Key not found. Please set the GOOGLE_API_KEY environment variable.")
else:
    try:
...
def extract_with_gemini(text):
    """
    Send founder materials to Gemini to extract structured company info and metrics.
    """
    prompt = f"""
You are an expert startup analyst. Extract key structured data from the following founder materials.
...
If any information is missing, leave the field blank. Use numeric values (no symbols like '$' or '%').
Text to analyze:
{text}
"""

    try:
        # Use the configured genai
        ...
        fields = [
            "company_name", "sector", "founded", "stage", "founder",
            "users_start", "users_end", "monthly_revenue", "monthly_churn", "new_hires"
        ]
        for f in fields:
            data.setdefault(f, "")
        ...
        st.error(f"⚠️ Gemini extraction failed, using fallback defaults. Error: {e}")
    ...
    if st.button("🔎 Analyze Startup"):
        ...
        try:
            ...
            if uploaded_files:
                for uploaded_file in uploaded_files:
                    ...
                    if file_type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                        ...
                        elements = partition(f"/tmp/{uploaded_file.name}")
            ...
            if not all_text.strip():
                st.warning("Please upload a file or paste some text before analyzing.")
                return

            with st.spinner("🤖 Extracting startup information using Gemini..."):
                extracted = extract_with_gemini(all_text)
...
def screen2():
    ...
    stage = data.get("stage", "Seed") or "Seed"
    founder = data.get("founder", "") or ""

    users_start = int(data.get("users_start") or 5000)
    users_end = int(data.get("users_end") or 7500)
    revenue = int(data.get("monthly_revenue") or 30000)
    churn_pct = float(data.get("monthly_churn") or 20)
    new_hires = int(data.get("new_hires") or 3)
    ...
    with st.sidebar:
        ...
        sector = st.text_input("Sector", value=sector)
        founded = st.text_input("Founded", value=founded)
        stage = st.selectbox("Stage", ["Pre-Seed", "Seed", "Series A", "Series B"], index=1)
        founder = st.text_input("Founder", value=founder)

        st.markdown("---")
        st.markdown("<h3>Last-Month Metrics</h3>", unsafe_allow_html=True)
        users_start = st.number_input("Users (start of month)", min_value=0, value=users_start, step=100)
        users_end = st.number_input("Users (end of month)", min_value=0, value=users_end, step=100)
        revenue = st.number_input("Monthly Revenue ($)", min_value=0, value=revenue, step=1000)
        churn_pct = st.slider("Monthly Churn (%)", 0, 100, int(churn_pct))
        new_hires = st.number_input("New Hires (last month)", min_value=0, value=new_hires, step=1)
    ...
    # st.caption("All these inputs are fully adjustable at runtime through the ⚙️ Controls section, allowing you to instantly explore different scenarios and outcomes.")
    st.markdown("""
    <p style='color: red;
              ...
              font-weight: bold;
              text-align: center;'>
    🔧 All these inputs are fully adjustable at runtime through the ⚙️ Controls section, allowing you to instantly explore different scenarios and outcomes.
    </p>
    """, unsafe_allow_html=True)
